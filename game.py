...
#        thread = threading.Thread(name='animate', target=a.update)
    ...
    def run(self):
        ...
        # NORTH 
        northButton = tk.Button(self.win, height="4", width="7", text="^", command=lambda: self.add('head'))
        northButton.grid(column=1, row=0, pady=5, padx=10) 
  
        # EAST 
        eastButton = tk.Button(self.win, height="4", width="7", text=">")
        eastButton.grid(column=3, row=2, pady=5, padx=10) 

        # SOUTH
        southButton = tk.Button(self.win, height="4", width="7", text="V")
        southButton.grid(column=2, row=3, pady=5, padx=10) 

        # WEST
        westButton = tk.Button(self.win, height="4", width="7", text="<")
        westButton.grid(column=1, row=2, pady=5, padx=10)

        # EXIT
        exitButton = tk.Button(self.win, height="2", width="7", text="Exit", command=sys.exit)
        exitButton.grid(column=5, row=0, pady=5, padx=10) 
